...
bool Win32Window::Create(const std::wstring& title,
  ...
  // ------------------------------------------
  // ⭐ Overlay mode: Layered window enables alpha blending
  // ------------------------------------------
  LONG style = GetWindowLong(window, GWL_EXSTYLE);
  style |= WS_EX_LAYERED;  // Enable per-pixel alpha
...
void Win32Window::SetChildContent(HWND content) {
  ...
  // ⭐ Critical: Apply layered style to Flutter child window
  LONG childStyle = GetWindowLong(child_content_, GWL_EXSTYLE);
  childStyle |= WS_EX_LAYERED;
  SetWindowLong(child_content_, GWL_EXSTYLE, childStyle);

  // Default alpha (255 = fully opaque)
  SetLayeredWindowAttributes(child_content_, 0, 255, LWA_ALPHA);
}
